...
(function () {
  ...
  function texteSansMetaImportFlair(texteBrut = '') {
    // Les champs UI d'import manuel (Source, contact, localisation) sont des
    // métadonnées. Ils peuvent enrichir la validation, mais ne doivent jamais
    // devenir le titre ou le nom d'entreprise du signal.
    const lignes = String(texteBrut || '').split(/\r?\n/);
    const metaRegex = /^\s*(?:origine\s+du\s+signal|source\s+terrain\s+directe|contact\s+rencontr[eé]|fonction\s+du\s+contact|localisation\s+connue)\s*[:：]/i;
    return lignes
      .filter(ligne => !metaRegex.test(ligne))
      .join('\n')
      .trim();
  }
  ...
  function codeDepartementDepuisCodePostal(codePostal = '') {
    const cp = String(codePostal || '').replace(/\D/g, '');
    if (!/^\d{5}$/.test(cp)) return '';
    if (cp.startsWith('97') || cp.startsWith('98')) return cp.slice(0, 3);
    if (cp.startsWith('20')) {
      // Corse : 20000/201xx plutôt Corse-du-Sud, 202xx plutôt Haute-Corse.
      return cp.startsWith('202') ? '2B' : '2A';
    }
    return cp.slice(0, 2);
  }

  function rechercherDepartementDepuisCodePostal(texteBrut = '', references = []) {
    const texte = String(texteBrut || '');
    if (!texte) return null;

    // Un code postal est accepté s'il est explicitement balisé ou isolé entre parenthèses.
    // Exemples : "Granville (50400)", "CP 50400", "code postal : 50400".
    // On évite ainsi de transformer un montant ou une année en géographie.
    const candidats = [];
    const patterns = [
      /\b(?:cp|code\s+postal|postal)\s*[:#-]?\s*(\d{5})\b/gi,
      /\((\d{5})\)/g
    ];

    for (const pattern of patterns) {
      for (const match of texte.matchAll(pattern)) {
        if (match?.[1]) candidats.push(match[1]);
      }
    }

    for (const cp of candidats) {
      const codeDep = codeDepartementDepuisCodePostal(cp);
      if (!codeDep) continue;
      const dep = references.find(item => String(item.departement_code || '').toUpperCase() === String(codeDep).toUpperCase());
      if (dep) return dep;
    }
  ...
  function rechercherDepartementGeneriqueDepuisTexte(texteBrut = '') {
    ...
    const references = [
      ...
      // Priorité aux libellés les plus longs : Val-d'Oise doit passer avant Oise,
      // Saône-et-Loire avant Loire, Indre-et-Loire avant Indre, etc.
      .sort((a, b) => {
        const maxLen = item => Math.max(
          String(item.departement_nom || '').length,
          ...((item.alias || []).map(alias => String(alias || '').length)),
          0
        );
        return maxLen(b) - maxLen(a);
      });

    const echapperRegex = value => String(value || '').replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const matcherTerme = value => {
      const cle = normaliserTexte(value);
      if (!cle) return false;
      return new RegExp(`(^|[^a-z0-9])${echapperRegex(cle)}([^a-z0-9]|$)`, 'i').test(normalise);
    };

    const toGeo = (item, source = 'flair_extraction_departement_generique') => ({
    ...
    // 1) Priorité absolue aux noms et alias textuels de départements.
    // Un nom explicite comme "Saône-et-Loire" doit passer avant tout code numérique.
    for (const item of references) {
      const aliasTextuels = [
        item.departement_nom,
        String(item.departement_nom || '').replace(/-/g, ' '),
        String(item.departement_nom || '').replace(/[’']/g, ' '),
        ...(item.alias || [])
      ].filter(Boolean);

      if (aliasTextuels.some(matcherTerme)) return toGeo(item);
    }

    const depDepuisCodePostal = rechercherDepartementDepuisCodePostal(texteBrut, references);
    if (depDepuisCodePostal) return toGeo(depDepuisCodePostal, 'flair_extraction_code_postal');

    // 2) Les codes départements ne sont acceptés que s'ils sont explicitement
    // présentés comme une géographie : "département 25", "Doubs (25)", "dept 25".
    // On refuse le code nu pour éviter "25 millions" => Doubs (25).
    for (const item of references) {
      const code = String(item.departement_code || '').trim();
      if (!code) continue;
      const codeRx = echapperRegex(normaliserTexte(code));
      const noms = [item.departement_nom, ...(item.alias || [])]
        .map(normaliserTexte)
        .filter(Boolean)
        .map(echapperRegex);

      const codeAvecContexte = new RegExp(`(?:departement|departement|dept|dep|dpt|code departement|code postal|cp)\\s*[:#-]?\\s*${codeRx}(?:[^a-z0-9]|$)`, 'i').test(normalise);
      const nomAvecCode = noms.length && new RegExp(`(?:${noms.join('|')})\\s*\\(?\\s*${codeRx}\\s*\\)?`, 'i').test(normalise);

      if (codeAvecContexte || nomAvecCode) return toGeo(item);
    }
  ...
  function extraireGeographieDepuisTexte(texteBrut = '') {
    ...
    // 1) Département explicite France entière : le département/code prime sur
    // une région seule. Cela évite de choisir un département par défaut dans
    // une région lorsque le texte donne déjà Saône-et-Loire, Mayenne, etc.
    ...
    // 2) Moteur géographique officiel flair-geo.js.
    const geoApi = getGeoApi();
  ...
  function nettoyerNomEntreprise(nomBrut = '') {
    ...
    const motsExclus = new Set([
      ...
      'suite', 'origine du', 'source', 'terrain', 'web ia'
    ]);
  ...
  function extraireEntrepriseDepuisTexte(texteBrut = '') {
    ...
    // FLAIR V2026.1 — information terrain : le début de phrase peut être
    // "Suite visite...". Dans ce cas, "Suite" n'est jamais une entreprise.
    // On cherche d'abord les marqueurs fiables : chez / visite de la société / échange avec.
    const patternsTerrain = [
      /\b(?:suite\s+(?:a|à)\s+)?(?:visite|rdv|rendez[ -]?vous|echange|échange)\s+(?:chez|avec|de|à|a)\s+(?:(?:la|le)\s+)?(?:soci[eé]t[eé]|entreprise|groupe)?\s*([A-ZÀ-ÖØ-Þ][A-Za-zÀ-ÖØ-öø-ÿ0-9&'’.\-]+(?:\s+[A-ZÀ-ÖØ-Þ][A-Za-zÀ-ÖØ-öø-ÿ0-9&'’.\-]+){0,5})\s+(?:à|a|en|dans|sur|avec|pour|qui|,|\.|$)/i,
      /\bchez\s+([A-ZÀ-ÖØ-Þ][A-Za-zÀ-ÖØ-öø-ÿ0-9&'’.\-]+(?:\s+[A-ZÀ-ÖØ-Þ][A-Za-zÀ-ÖØ-öø-ÿ0-9&'’.\-]+){0,5})\s+(?:à|a|en|dans|sur|avec|pour|qui|,|\.|$)/i,
      /\b(?:soci[eé]t[eé]|entreprise|groupe)\s+([A-ZÀ-ÖØ-Þ][A-Za-zÀ-ÖØ-öø-ÿ0-9&'’.\-]+(?:\s+[A-ZÀ-ÖØ-Þ][A-Za-zÀ-ÖØ-öø-ÿ0-9&'’.\-]+){0,5})\s+(?:à|a|en|dans|sur|avec|pour|qui|,|\.|$)/i
    ];

    for (const pattern of patternsTerrain) {
  ...
  function detecterTypeSignalDepuisTexte(texteBrut = '') {
    ...
    const appelOffresNie = phrases.some(p =>
      /(aucun|aucune|pas de|pas d |sans|non annonce|non lance|pas encore|n est pas|n est mentionne)/i.test(p) &&
      /(appel d offres|appel d offre|dce|marche public|marche public)/i.test(p)
    );
    const appelOffresActuel = phrases.some(p =>
      /(appel d offres en cours|appel d offre en cours|appel d offres publie|appel d offre publie|dce publie|marche public publie)/i.test(p)
    );
    if (!appelOffresNie && appelOffresActuel) return 'appel_offre';
  ...
  function construireSignalAnalyse(texteBrut = '') {
    const texte = String(texteBrut || '').trim();
    const texteMetier = texteSansMetaImportFlair(texte) || texte;

    const entreprise = extraireEntrepriseDepuisTexte(texteMetier);
    // La géographie peut exploiter le texte complet, car "Localisation connue"
    // est une aide volontaire du commercial ; l'entreprise et le titre, eux,
    // restent extraits du texte métier uniquement.
    const geographie = extraireGeographieDepuisTexte(texte);
    const typeSignal = detecterTypeSignalDepuisTexte(texteMetier);
    const maturite = detecterMaturiteCommerciale(texte);
    const titre = construireTitreDepuisExtraction(texteMetier, entreprise, typeSignal);
    const texteNorm = normaliserTexte(texte);
    const negationCommerciale = /(?:aucun|aucune|pas de|pas d |sans|non annonce|non lance|n est pas|n est mentionne)/i.test(texteNorm);

    return {
      entreprise: { nom: entreprise, confiance: entreprise ? 90 : 0 },
      projet: {
        titre,
        type_signal: typeSignal,
        maturite,
        preuves: {
          nouvelle_ligne: typeSignal === 'nouvelle_ligne',
          investissement: typeSignal === 'investissement' || texteNorm.includes('investissement'),
          consultation_niee: negationCommerciale && /(?:consultation|consultations|fournisseur|fournisseurs|demande de devis|demande de prix)/i.test(texteNorm),
          appel_offres_nie: negationCommerciale && /(?:appel d offres|appel d offre|dce|marche public)/i.test(texteNorm)
        }
      },
      geographie,
      texte_metier: texteMetier,
      texte_original: texte
    };
  }

  function analyserTexteImporte(texteBrut = '') {
    const analyse = construireSignalAnalyse(texteBrut);

    return {
      entreprise_nom: analyse.entreprise.nom,
      geographie: analyse.geographie,
      type_signal: analyse.projet.type_signal,
      maturite: analyse.projet.maturite,
      titre: analyse.projet.titre,
      signal_analyse: analyse,
      texte_original: analyse.texte_original
    };
  }

  function appliquerTimingExtraction(timing = {}, extraction = null) {
    // Pipeline unique FLAIR V2026.1 : ce module n'impose plus de timing.
    // Les indices détectés restent disponibles dans extraction.maturite_extraction_flair,
    // mais la décision commerciale est prise uniquement par flair-metier.js.
    return timing;
  }
  ...
  function enrichirSignalScoring(signal = {}, extraction = null) {
    ...
    return {
      ...
      signal_analyse_flair: extraction.signal_analyse || null,
      consultation_niee: extraction.signal_analyse?.projet?.preuves?.consultation_niee === true,
      appel_offres_nie: extraction.signal_analyse?.projet?.preuves?.appel_offres_nie === true
    };
  }

  window.FLAIR_EXTRACTION = {
    analyserTexteImporte,
    construireSignalAnalyse,
