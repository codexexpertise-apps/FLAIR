(function () {
  ...
  function validerStructure(doctrine) {
    ...
    const pipeline = lireChemin(doctrine, 'bloc_D.pipeline_ordre_obligatoire');
    ...
    if (!Array.isArray(pipeline) || pipeline.length !== 12) {
  ...
  function evaluerFraicheur(signal = {}, source = '') {
    ...
    const ouverture = [
      ...
      'd ici 2026', 'd ici 2027', 'en 2026', 'en 2027', 'a venir', 'prochainement'
    ].some(indice => corpus.includes(indice));
  ...
  function verifierAvantScoring(signal = {}, source = '') {
    ...
    if (!sourceTexte && !signal.source_url && !signal.url_source) nonConformites.push({ code: 'PREUVE_ABSENTE', criticite: 'majeure', message: 'Aucune preuve ou source exploitable n’est disponible.' });
