(function () {
  ...
  function extraireAutoriteScoring(doctrine = {}) {
    return {
      autorite: lireChemin(doctrine, 'regles_machine.scoring.autorite_unique')
        || lireChemin(doctrine, 'blocs.E.articles.E9.autorite_scoring')
        || '',
      collecteIgnoree: lireChemin(doctrine, 'regles_machine.scoring.score_collecte_ignore') === true
        || lireChemin(doctrine, 'blocs.E.articles.E9.score_collecte_ignore') === true,
      sourceIgnoree: lireChemin(doctrine, 'regles_machine.scoring.score_source_ignore') === true
        || lireChemin(doctrine, 'blocs.E.articles.E9.score_collecte_ignore') === true
    };
  }
  ...
  function validerStructure(doctrine) {
    ...
    const autoriteScoring = extraireAutoriteScoring(doctrine);
    ...
    if (autoriteScoring.autorite !== 'moteur_flair' || !autoriteScoring.collecteIgnoree || !autoriteScoring.sourceIgnoree) {
      erreurs.push('La doctrine doit désigner le moteur FLAIR comme autorité unique de scoring et imposer l’ignorance des scores de collecte ou de source.');
    }

    return { valide: erreurs.length === 0, erreurs, pipeline, autoriteScoring };
  }
