...
async function sauvegarderOnboardingMetier() {
  ...
  // - Sans invitation/team_id : l'utilisateur crée son propre espace FLAIR et devient manager de son espace.
  const roleFinal = payloadInvitation.team_id
    ? (payloadInvitation.role || profilActuel?.role || role)
    : 'manager';

  const fonctionFinale = payloadInvitation.team_id
    ? fonction
    : (roleFinal === 'manager' ? 'manager_commercial' : fonction);
...
async function afficherVue(vue) {
  ...
  const managerSubViews = ['manager', 'sources', 'relances', 'qualite'];
