...
function extraireSignalDepuisArticle(texteBrut) {
  ...
  // Doctrine V2026.2 : un score éventuellement présent dans la collecte est
  // détecté uniquement pour compatibilité et diagnostic. Il n'entre jamais
  // dans le calcul : le moteur FLAIR reste l'unique autorité de scoring.
  const scoreCollecteIgnore = extraireScoreImport(texte);
  if (scoreCollecteIgnore !== null) {
    console.info(`FLAIR — score de collecte ignoré (${scoreCollecteIgnore}/100) ; recalcul complet par le moteur FLAIR.`);
  }
  ...
  const scoreFinal = Math.max(0, Math.min(100, Number(resultatEnrichi.score_pertinence) || 0));
  ...
  const signalFinalImporte = {
    ...
    chaleur: resultatEnrichi.chaleur || chaleurDepuisScoreFlair(scoreFinal),
