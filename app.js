...
async function chargerContexteManagerRadar() {
  ...
  const { data: signauxData, error: signauxError } = await supabaseClient
    .from('signaux')
    .select('*')
    .eq('team_id', teamId);
  ...
  let distributions = [];
  const { data: distributionsData, error: distributionsError } = await supabaseClient
  ...
  let recherchesIa = [];
  const { data: recherchesData, error: recherchesError } = await supabaseClient
