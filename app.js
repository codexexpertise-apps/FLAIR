...
function extraireSignalDepuisArticle(texteBrut) {
  ...
  const extractionFlair = window.FLAIR_EXTRACTION?.analyserTexteImporte
  ...
  if (window.FLAIR_EXTRACTION?.appliquerTimingExtraction) {
    timing = window.FLAIR_EXTRACTION.appliquerTimingExtraction(timing, extractionFlair);
  }
  ...
  return corrigerInferenceSemantiqueFlair(signalFinalImporte, texte);
}
...
async function upserterDistributionsCommercialesAvecFallback(lignes = [], options = {}) {
  ...
  const permissionNonBloquante =
    ...
    options.sourceDistribution === 'manuel';
...
async function analyserArticleImporte() {
  ...
  signalImporte = corrigerInferenceSemantiqueFlair(signalImporte, texte);
