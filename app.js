...
function setManagerHeaderForSubview(view) {
  ...
  const configs = {
    manager: ['Dashboard PRO', `Bienvenue ${currentProfil?.prenom || ''}, voici votre vision manager`],
    sources: ['Sources & veille', 'Piloter les sources qui alimentent le radar'],
    ...
    qualite: ['Qualité du radar', 'Mesurer la pertinence et la transformation du radar']
  };
...
function renderManagerSourcesVeille(ctx, container) {
  ...
  const sourcesActives = (ctx.sourcesVeille || []).filter(s => s.actif === true || s.actif === 'true');
  ...
  container.innerHTML = `
    ...
    <section class="manager-panel manager-sources-lecture-panel">
      ...
      <div class="manager-bars-list" style="margin-top:14px;">
        ${sourcesActives.length ? sourcesActives.slice(0, 6).map(s => `
          <div class="manager-source-row">
            <div>
              <div class="manager-source-title">${managerEscape(s.nom || 'Source sans nom')}</div>
              <div class="manager-source-sub">${managerEscape(s.type_source || 'type non renseigné')} · fréquence : ${managerEscape(s.frequence_scan || 'non définie')}</div>
            </div>
...
function renderCoordonneesEntreprise(s = {}, options = {}) {
  ...
  if (!site && !tel && !email) return '';

  const lignes = [];
  if (site) lignes.push(`<small><b>🌐 Site web :</b> ${managerEscape(site)}</small>`);
  if (tel) lignes.push(`<small><b>☎ Téléphone standard :</b> ${managerEscape(tel)}</small>`);
  if (email) lignes.push(`<small><b>✉ Email générique :</b> ${managerEscape(email)}</small>`);

  const titre = options.titre || 'Coordonnées publiques entreprise';
  return `
    <div style="margin:8px 0;padding:9px 11px;border:1px solid rgba(148,163,184,0.30);background:rgba(15,23,42,0.06);border-radius:10px;">
      <b>${managerEscape(titre)}</b><br>
      ${lignes.join('<br>')}
    </div>
  `;
}
...
function construireLigneTableOpportunite(s = {}, contexte = 'commercial') {
  ...
  return `
    <tr>
      ...
      <td><small>${managerEscape(signalSiteWeb(s) || '—')}</small><br><small>${managerEscape(signalTelephoneStandard(s) || '')}</small><br><small>${managerEscape(signalEmailGenerique(s) || '')}</small></td>
      <td>${sourceCell}</td>
    </tr>
  `;
}
...
async function exporterListeOpportunitesCommerciales() {
  ...
  if (!window.FLAIR_EXPORT_UTILS?.exporterSignauxEnCSV) {
    alert('Module export CSV indisponible.');
    return;
  }
  ...
  const nomCommercial = currentProfil
    ...
    : 'FLAIR';
  window.FLAIR_EXPORT_UTILS.exporterSignauxEnCSV(data || [], `Opportunites_${nomCommercial || 'FLAIR'}`);
}
...
function renderManagerListeOpportunites(ctx, container) {
  ...
  container.innerHTML = `
    <section class="manager-panel" style="grid-column:1 / -1;">
      <div class="manager-panel-head">
        <h4>Liste d’opportunités équipe</h4>
        <span class="manager-mini-badge">Excel / CSV</span>
      </div>
...
async function exporterListeOpportunitesManager() {
  try {
    ...
    window.FLAIR_EXPORT_UTILS.exporterSignauxEnCSV(signaux, 'Opportunites_Equipe_FLAIR');
  } catch (err) {
...
function extraireSignalDepuisArticle(texteBrut) {
  ...
  const secteur = detecterSecteurSousSecteur(signalScoring);
  const timing = calculerTimingCommercial(signalScoring, resultatEnrichi);
...
function renderBlocCopilotePremium(copiloteJson = {}) {
  ...
  return `
    <div style="margin:10px 0;padding:13px 14px;border:1px solid rgba(59,130,246,0.45);background:linear-gradient(180deg,rgba(59,130,246,0.13),rgba(15,23,42,0.10));border-radius:14px;box-shadow:0 10px 26px rgba(15,23,42,0.12);">
      ...
      ${copiloteJson.coordonnees_publiques ? renderCoordonneesEntreprise(copiloteJson.coordonnees_publiques, { titre: 'Coordonnées publiques entreprise' }) : ''}
...
function renderBlocCopiloteCommercial(s = {}) {
  ...
  const coordonnees = renderCoordonneesEntreprise(s, { titre: 'Coordonnées publiques entreprise' });
