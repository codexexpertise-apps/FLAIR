...
(function () {
  ...
  function detecterGuardrailSemantiqueFlair(signal = {}, timing = {}) {
    ...
    const consultationProspective = phrases.some(phrase =>
      phraseContientUnIndiceFlair(phrase, [
        ...
        'consultations sont susceptibles d intervenir',
        ...
        'consultations fournisseurs pourraient intervenir',
        'consultations fournisseurs devraient intervenir',
        ...
        'choix techniques sont susceptibles d intervenir'
      ])
    );

    const consultationEtAppelOffresNies = appelOffresNie && consultationNiee;
    const interditUrgence = consultationEtAppelOffresNies || chantierNie || phaseAmontForte;
  ...
  function detecterPreuvesCredibiliteFlair(signal = {}, timing = {}) {
    ...
    if (
      !gardeFou.appel_offres_nie && (
        signal.type_signal === 'appel_offre' ||
        contientIndiceCredibiliteFlair(texte, [
        'appel d offres', 'appel d offre', 'appel offres', 'dce publie',
        'dce publié', 'marche public', 'marché public', 'boamp'
        ])
      )
    ) {
    ...
    if (
      ...
      contientIndiceCredibiliteFlair(texte, [
        'consultation en cours', 'consultation fournisseurs', 'consultations fournisseurs',
        'demande de prix', 'demande de devis', 'fournisseurs seront consultes',
        'fournisseurs seront consultés', 'selection fournisseurs', 'sélection fournisseurs'
      ])
    ) {
      ajouter('consultation', 'Consultation fournisseurs ou demande de prix');
    }
  ...
  function plafondCredibiliteDepuisPreuvesFlair(preuves = [], signal = {}, timing = {}) {
    const nb = Array.isArray(preuves) ? preuves.length : 0;
  ...
  function appliquerRegleCredibiliteFlair(signal = {}, scores = {}) {
    ...
    const plafond = plafondCredibiliteDepuisPreuvesFlair(preuves, signal, timing);
  ...
  function calculerScoresSeparationFlair(options = {}) {
    ...
    const credibilite = appliquerRegleCredibiliteFlair(signal, {
      score: scoreFinalMetier,
      score_final_distribue: scoreFinalMetier,
      timing
    });
    const prioriteTiming = ajusterScoreSelonPrioriteTimingFlair(
      credibilite.score,
      timing,
      credibilite.plafond_credibilite
    );
    const scoreFinal = prioriteTiming.score;
